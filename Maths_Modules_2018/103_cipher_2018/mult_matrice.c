...
int **mult_matrice_f(int **matrice1, float **matrice2)
{
    ...
    matrice_base = malloc(sizeof(float *) * nb_lin1);
    matrice_resultat = malloc(sizeof(int *) * nb_lin1);
    for (int i = 0; i < nb_lin1; i++) {
        matrice_base[i] = malloc(sizeof(float) * nb_lin1);
        matrice_resultat[i] = malloc(sizeof(int) * nb_col1);
        for (int j = 0; j < nb_col1; j++) {
           matrice_base[i][j] = 0.0;
            for (int k = 0; k < nb_lin2; k++) {
                matrice_base[i][j] += matrice1[i][k] * matrice2[k][j];
            }
        }
    }
    for (int i = 0; i < nb_lin1; i++) {
        for (int j = 0; j < nb_col1; j++) {
            matrice_resultat[i][j] = (int)(matrice_base[i][j] + 0.1);
        }
    }
    return (matrice_resultat);
}
