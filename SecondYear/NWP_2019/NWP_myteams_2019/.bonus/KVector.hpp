...
public:
    ...
    void eraseAt(size_t index) { this->at(index); std::vector<T>::erase(this->begin() + index); }
    ...
    void removeLast() { erase(this->rbegin()); }

    /**
     * @brief Takes object at index and removes it from the vector.
     * @param index
     * @return T
     */
    T takeAt(size_t index) { T obj = (*this)[index]; removeAt(index); return obj; }
    T takeAt(typename KVector<T>::iterator index) { T obj = *index; removeAt(index); return obj; }
    /**
     * @brief Takes nb objects starting with the object at index, returns them
     * and removes them of the vector.
     * @param nb
     * @param pos
     * @return KVector<T>
     */
    KVector<T> take(size_t nb, size_t index = 0) { KVector<T> taken; for (size_t i = 0; i < nb; ++i) taken.push_back((*this)[i + index]); remove(index, index + nb); return taken; }
